    ...
    <main className="min-h-dvh overflow-x-hidden bg-[#120f0b] px-4 py-6 text-white sm:px-6 lg:py-8">
      ...
      <div className="relative mx-auto grid max-w-6xl items-start gap-8 lg:min-h-[calc(100dvh-4rem)] lg:grid-cols-[0.9fr_1fr] lg:items-center">
        <section>
          ...
          <h1 className="max-w-xl text-5xl font-extrabold leading-[0.98] tracking-normal sm:text-6xl">Set up your BorrowBoard profile.</h1>
        ...
        <form onSubmit={handleSubmit} className="rounded-3xl border border-white/12 bg-white/[0.07] p-5 shadow-2xl shadow-black/35 backdrop-blur-2xl sm:p-7">
          <div className="mb-6">
            <p className="text-xs font-extrabold uppercase tracking-wide text-amber-100/70">A few quick questions</p>
            <h2 className="mt-2 text-3xl font-extrabold">Finish your account</h2>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="block">
                ...
                <select value={grade} onChange={(event) => setGrade(Number(event.target.value))} className="mt-2 w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-sm font-semibold text-white outline-none transition focus:border-amber-300 focus:ring-4 focus:ring-amber-200/15">
              ...
              <label className="block">
                ...
                <input value={studentId} onChange={(event) => setStudentId(event.target.value)} required className="mt-2 w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-sm font-semibold text-white outline-none transition placeholder:text-stone-500 focus:border-amber-300 focus:ring-4 focus:ring-amber-200/15" placeholder="STU-10492" />
              </label>
            </div>
            ...
            <label className="block">
              ...
              <input value={school} onChange={(event) => setSchool(event.target.value)} required className="mt-2 w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-sm font-semibold text-white outline-none transition placeholder:text-stone-500 focus:border-amber-300 focus:ring-4 focus:ring-amber-200/15" placeholder="Westview High School" />
            </label>

            <label className="block">
              <span className="text-xs font-bold uppercase tracking-wide text-stone-300">Default pickup spot</span>
              <select value={pickupLocation} onChange={(event) => setPickupLocation(event.target.value)} className="mt-2 w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-sm font-semibold text-white outline-none transition focus:border-amber-300 focus:ring-4 focus:ring-amber-200/15">
                {['Library', 'Cafeteria', 'Room 210', 'STEM Lab', 'Gym', 'Main Office'].map((location) => <option key={location}>{location}</option>)}
              </select>
            </label>

            <label className="group block cursor-pointer rounded-3xl border border-dashed border-amber-100/25 bg-black/20 p-4 transition hover:border-amber-200/60 hover:bg-amber-100/[0.06]">
              <input type="file" accept=".txt,.csv,text/plain,text/csv" onChange={handleScheduleUpload} className="sr-only" />
              <span className="flex items-start gap-3">
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-amber-100 text-stone-950 shadow-lg shadow-amber-500/20">
          ...
          <button type="submit" className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-white px-5 py-3 text-sm font-extrabold text-stone-950 transition hover:bg-amber-100">
